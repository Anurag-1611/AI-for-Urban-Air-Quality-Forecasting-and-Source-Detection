...
st.markdown(
    ...
    .nav-pill { width:100%; max-width:980px; margin: 0 auto 18px auto; background: linear-gradient(90deg,#111111,#222222); padding: 10px 18px; border-radius: 999px; box-shadow: 0 12px 30px rgba(0,0,0,0.25); display:flex; align-items:center; justify-content:space-between; gap:12px; }
    .brand { display:flex; align-items:center; gap:10px; color:#fff; font-weight:800; font-size:16px; }
    .nav-pill-right { background: white; color: #0b3e2c; padding:8px 14px; border-radius:999px; font-weight:700; box-shadow: 0 8px 18px rgba(0,0,0,0.12); }
    ...
    .stButton>button { background: linear-gradient(90deg,#10b981,#047857) !important; color:white !important; font-weight:800 !important; padding:12px 30px !important; border-radius:12px !important; font-size:18px !important; }
...
# ---------- Load models helper (root-first, fallback to models/) ----------
@st.cache_resource
def load_models():
    """
    Tries to load the three model files from project root first.
    If not found there, it also tries ./models/ (backward compatibility).
    """
    ...
    # prefer root files (same folder as app), fallback to models/ subfolder
    candidates = {
        'pollution_model': ['rf_pollution_likely.pkl', os.path.join('models', 'rf_pollution_likely.pkl')],
        'source_model': ['rf_source.pkl', os.path.join('models', 'rf_source.pkl')],
        'source_le': ['label_encoder_source.pkl', os.path.join('models', 'label_encoder_source.pkl')]
    }
    ...
    for k, paths in candidates.items():
        ...
        out[k] = loaded
        if not loaded:
            out['errors'].append(f"Could not load {k}. Tried: {paths}. Errors: {errs}")
    return out
...
# Debug info about model loading
...
if predict_clicked:
    ...
    try:
        ...
        if poll_model is None:
            st.error("Pollution model not loaded. Place 'rf_pollution_likely.pkl' next to app_rf.py (project root) and restart.")
        else:
...
st.markdown("<div style='text-align:center;color:#064e3b;font-size:12px;margin-top:12px'>Place rf_pollution_likely.pkl, rf_source.pkl, label_encoder_source.pkl next to this app file (project root).</div>", unsafe_allow_html=True)
